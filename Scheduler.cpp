...
void Scheduler::execute_job(lua_State *runOn, SchedulerJob *job) {
    ...
    memcpy(nLs->userdata, runOn->userdata, 0x98);   // Copy extra space of runon into nLs, both pointers are guaranteed to be valid.
...
bool Scheduler::is_initialized() const { return this->m_lsInitialisedWith != nullptr && this->m_lsRoblox != nullptr; }
