...
static std::uintptr_t luau_execute = RebaseAddress(0x14340e320);

// Get to lua_pushvalue and get inside of pseudo2addr, you will find the data xref.
const static std::uintptr_t _luaO_nilobject = RebaseAddress(0x146bc1e58);

// Find " ,"metatable": ", and go to the top of the second xref, a ldebug
// function. It will contain two references to luaH_dummynode.
const static std::uintptr_t _luaH_dummynode = RebaseAddress(0x146bc1d18);
...
static std::uintptr_t pseudo2addr = RebaseAddress(0x1433ffda0);
...
static std::uintptr_t rluaE_newthread = RebaseAddress(0x143401290);
...
static std::uintptr_t rLuaD_throw = RebaseAddress(0x143405080);
...
static std::uintptr_t rlua_newthread = RebaseAddress(0x1433fdd10);

// search for "defer"
static std::uintptr_t rTask_defer = RebaseAddress(0x141fd1f20);
...
static std::uintptr_t rFreeBlock = RebaseAddress(0x14341b4b0);
...
static std::uintptr_t rFromLuaState = RebaseAddress(0x141e29a50);
...
static std::uintptr_t rLuaD_rawrununprotected = RebaseAddress(0x143404e30);
...
// ? 41 F7 E8 8B FA C1 FF ? 8B C7 C1 E8 ? 03 F8 48 8B 83 ? ? ? ? 48 85 C0 74 04
static std::uintptr_t rLuaC_Step = RebaseAddress(0x143418e30);

// search for "ProximityPrompt_Triggered".
static std::uintptr_t fireproximityprompt = RebaseAddress(0x1427fa3e0);

// search for "InvalidInstance". Caller with two arguments (Modifies lua stack)
static std::uintptr_t pushinstance = RebaseAddress(0x141cd2230);

// Search for "TaskSchedulerTargetFps". Use the .data segment reference it refers towards. It is a DWORD (or int32_t)
const static std::uintptr_t FFlag_TaskSchedulerTargetFps = RebaseAddress(0x148736ff4);
