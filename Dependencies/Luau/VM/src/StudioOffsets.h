...
namespace RBX::Studio::Offsets {
    ...
    static std::uintptr_t luau_execute = RebaseAddress(0x14323efd0);

    // Get to lua_pushvalue and get inside of pseudo2addr, you will find the data xref.
    const static std::uintptr_t _luaO_nilobject = RebaseAddress(0x146960040);

    // Find " ,"metatable": ", and go to the top of the second xref, a ldebug
    // function. It will contain two references to luaH_dummynode.
    const static std::uintptr_t _luaH_dummynode = RebaseAddress(0x14695fee8);
    ...
    static std::uintptr_t pseudo2addr = RebaseAddress(0x1432315a0);
    ...
    static std::uintptr_t rluaE_newthread = RebaseAddress(0x143232a90);
    ...
    static std::uintptr_t rLuaD_throw = RebaseAddress(0x143236570);
    ...
    static std::uintptr_t rlua_newthread = RebaseAddress(0x14322f510);

    // search for "defer"
    static std::uintptr_t rTask_defer = RebaseAddress(0x141e8af10);
    ...
    static std::uintptr_t rFreeBlock = RebaseAddress(0x14324c390);
    ...
    static std::uintptr_t rFromLuaState = RebaseAddress(0x141ccf3d0);
    ...
    static std::uintptr_t rLuaD_rawrununprotected = RebaseAddress(0x143236320);
    ...
    static std::uintptr_t rLuaC_Step = RebaseAddress(0x143249c60);

    // search for "ProximityPrompt_Triggered".
    static std::uintptr_t fireproximityprompt = RebaseAddress(0x1426804b0);

    // search for "InvalidInstance". Caller with two arguments (Modifies lua stack)
    static std::uintptr_t pushinstance = RebaseAddress(0x141db7b90);

    // Search for "TaskSchedulerTargetFps". Use the .data segment reference it refers towards. It is a DWORD (or int32_t)
    const static std::uintptr_t FFlag_TaskSchedulerTargetFps = RebaseAddress(0x14840d108);
