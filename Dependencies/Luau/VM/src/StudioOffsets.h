...
static std::uintptr_t luau_execute = RebaseAddress(0x143681EB0);

// Get to lua_pushvalue and get inside of pseudo2addr, you will find the data xref.
const static std::uintptr_t _luaO_nilobject = RebaseAddress(0x146E98758);

// Find " ,"metatable": ", and go to the top of the second xref, a ldebug
// function. It will contain two references to luaH_dummynode.
const static std::uintptr_t _luaH_dummynode = RebaseAddress(0x146E98618); 
...
static std::uintptr_t pseudo2addr = RebaseAddress(0x14366A2D0);
...
static std::uintptr_t rluaE_newthread = RebaseAddress(0x14366B7C0);
...
static std::uintptr_t rLuaD_throw = RebaseAddress(0x143678EF0);
...
static std::uintptr_t rlua_newthread = RebaseAddress(0x143668230);

// search for "defer"
static std::uintptr_t rTask_defer = RebaseAddress(0x142122BC0);
...
static std::uintptr_t rFreeBlock = RebaseAddress(0x143685500);
...
static std::uintptr_t rFromLuaState = RebaseAddress(0x141E10520);
...
static std::uintptr_t rLuaD_rawrununprotected = RebaseAddress(0x143677B20);
...
static std::uintptr_t rLuaC_Step = RebaseAddress(0x143682EA0);

// search for "ProximityPrompt_Triggered".
static std::uintptr_t fireproximityprompt = RebaseAddress(0x142A24EC0);

// search for "InvalidInstance". Caller with two arguments (Modifies lua stack)
static std::uintptr_t pushinstance = RebaseAddress(0x141E92850);

// Search for "TaskSchedulerTargetFps". Use the .data segment reference it refers towards. It is a DWORD (or int32_t)
const static std::uintptr_t FFlag_TaskSchedulerTargetFps = RebaseAddress(0x0);
} // namespace RBX::Studio::Offsets
