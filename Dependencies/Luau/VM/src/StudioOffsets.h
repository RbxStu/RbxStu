...
namespace RBX::Studio::Offsets {
    ...
    static std::uintptr_t luau_execute = RebaseAddress(0x1433bf890);

    // Get to lua_pushvalue and get inside of pseudo2addr, you will find the data xref.
    const static std::uintptr_t _luaO_nilobject = RebaseAddress(0x146b4ecf0);

    // Find " ,"metatable": ", and go to the top of the second xref, a ldebug
    // function. It will contain two references to luaH_dummynode.
    const static std::uintptr_t _luaH_dummynode = RebaseAddress(0x146b4eb98);
    ...
    static std::uintptr_t pseudo2addr = RebaseAddress(0x1433b1300);
    ...
    static std::uintptr_t rluaE_newthread = RebaseAddress(0x1433b27f0);
    ...
    static std::uintptr_t rLuaD_throw = RebaseAddress(0x1433b65f0);
    ...
    static std::uintptr_t rlua_newthread = RebaseAddress(0x1433af270);

    // search for "defer"
    static std::uintptr_t rTask_defer = RebaseAddress(0x141f971c0);
    ...
    static std::uintptr_t rFreeBlock = RebaseAddress(0x1433cca20);
    ...
    static std::uintptr_t rFromLuaState = RebaseAddress(0x141dee730);
    ...
    static std::uintptr_t rLuaD_rawrununprotected = RebaseAddress(0x1433b63a0);
    ...
    static std::uintptr_t rLuaC_Step = RebaseAddress(0x1433ca3a0);

    // search for "ProximityPrompt_Triggered".
    static std::uintptr_t fireproximityprompt = RebaseAddress(0x1427bd430);

    // search for "InvalidInstance". Caller with two arguments (Modifies lua stack)
    static std::uintptr_t pushinstance = RebaseAddress(0x141cc6200);

    // Search for "TaskSchedulerTargetFps". Use the .data segment reference it refers towards. It is a DWORD (or int32_t)
    const static std::uintptr_t FFlag_TaskSchedulerTargetFps = RebaseAddress(0x1486ab1c4);
